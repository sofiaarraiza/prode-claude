...
import { supabase, type Match, type Prediction, type Group } from '@/lib/supabase'
import { GROUPS, isMatchEditable } from '@/lib/fixture'
import BottomNav from '@/components/layout/BottomNav'
import { format, parseISO } from 'date-fns'
import { es } from 'date-fns/locale'

type PredictionMap = Record<string, { home: string; away: string; saved?: boolean; points?: number }>
...
export default function PrediccionesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: matchData }, { data: memberData }] = await Promise.all([
        supabase.from('matches').select('*').eq('phase', 'group').order('match_number'),
  ...
  const savePrediction = async (match: Match) => {
    ...
    if (!error) {
      setPredictions(prev => ({ ...prev, [match.id]: { ...prev[match.id], saved: true } }))
    }
  ...
  const getPointsBadge = (matchId: string, match: Match) => {
    ...
    if (!pred) return <span className="text-xs bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">Sin pred.</span>
    if (pred.points === 3) return <span className="text-xs bg-[#003DA5] text-white px-2 py-0.5 rounded-full font-bold">+3 pts 🎯</span>
    if (pred.points === 1) return <span className="text-xs bg-amber-400 text-white px-2 py-0.5 rounded-full font-bold">+1 pt</span>
    return <span className="text-xs bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">0 pts</span>
  }
  ...
  return (
    <div className="min-h-dvh bg-[#F0F4FF] pb-24">
      ...
      <div className="bg-fifa-pattern px-5 pt-14 pb-4">
        <p className="text-white/60 text-xs tracking-widest mb-1">COPA DEL MUNDO 2026</p>
        <h1 className="text-white font-display text-3xl mb-4" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>
          MIS PREDICCIONES
        </h1>
        ...
        {groups.length > 0 ? (
          ...
          <div className="bg-white/10 rounded-xl p-3 text-white/70 text-xs">
            ...
            <button onClick={() => router.push('/grupos')} className="underline text-white font-semibold">Crear o unirte</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-20">
        <div className="overflow-x-auto flex px-4 py-2 gap-1" style={{ scrollbarWidth: 'none' }}>
          {GROUPS.map(g => (
            <button
              key={g}
      ...
      <div className="px-4 py-4 space-y-3">
        {groupMatches.map((match) => {
          ...
          const hasUnsaved = pred && !pred.saved && pred.home !== '' && pred.away !== ''
              ...
              <div className="px-4 py-4">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 text-center">
                    ...
                    <p className="text-xs font-semibold text-gray-700 leading-tight">{match.home_team}</p>
                  </div>
                    ...
                    {status === 'finished' ? (
                      ...
                      <div className="text-center">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-display text-3xl text-[#003DA5]" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>
                            {match.home_score ?? '-'}
                          </span>
                        ...
                        {pred && (
                          <p className="text-xs text-gray-400">
                            Tu pred: <strong>{pred.home}-{pred.away}</strong>
                          </p>
                        )}
                      </div>
                    ) : (
                  ...
                  <div className="flex-1 text-center">
                    ...
                    <p className="text-xs font-semibold text-gray-700 leading-tight">{match.away_team}</p>
                  </div>
                </div>
                ...
                {status === 'open' && selectedGroup && (
                  <div className="mt-3 flex items-center justify-center">
                    {isSaving ? (
                      ...
                      <span className="text-xs text-gray-300">Ingresá tu predicción</span>
                    )}
                  </div>
                )}
              </div>
