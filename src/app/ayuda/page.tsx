...
const SECTIONS = [
  {
    ...
    content:
      "El prode del Mundial 2026 es un juego de predicciones donde vos predecís el resultado de cada partido antes de que empiece. Competís contra tus amigos dentro de un grupo y gana quien más puntos junte.",
  },
  {
    emoji: "⚽",
    ...
    rows: [
      {
        ...
        desc: "Predecís el marcador exacto. Ej: predecís 2-1 y sale 2-1. ¡Máxima puntuación!",
      },
      {
        badge: "1",
        color: "#F59E0B",
        title: "Ganador o empate",
        desc: "Acertás quién gana o que empatan, pero no el marcador exacto. Ej: predecís 2-1 y sale 3-0.",
      },
      {
        badge: "0",
        color: "#E5E7EB",
  ...
  },
  {
    ...
    content:
      "Gana quien más puntos acumule al final del torneo. En caso de empate, gana quien tenga más resultados exactos (3 pts). El sistema de premios o apuestas se arregla por fuera de la app entre los participantes.",
  },
  {
    emoji: "🌍",
    title: "¿Cuándo empieza el Mundial?",
    content:
      "La Copa del Mundo FIFA 2026 arranca el 11 de junio de 2026 con México 🇲🇽 vs Sudáfrica 🇿🇦 en el Estadio Azteca. El torneo incluye 48 selecciones distribuidas en 12 grupos, con 104 partidos en total.",
  },
];
    ...
    <div className="min-h-dvh bg-app pb-10">
      ...
      <div className="px-4 py-5 space-y-4">
        {SECTIONS.map((section, i) => (
          <div key={i} className="bg-surface rounded-3xl shadow-sm p-5">
            <div className="flex items-center gap-3 mb-3">
              <span className="text-2xl">{section.emoji}</span>
              <h2 className="font-bold text-[color:var(--color-text)] text-base">
                {section.title}
              </h2>
            </div>

            {section.content && (
              <p className="text-[color:var(--color-text-2)] text-sm leading-relaxed">
                {section.content}
              </p>
            )}
                ...
                {section.rows.map((row, j) => (
                  <div key={j} className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[color:var(--color-muted)] text-xs mt-0.5 leading-relaxed">
                        {row.desc}
                      </p>
                    </div>
                  </div>
        ...
        <div className="bg-surface rounded-3xl shadow-sm p-5">
          <h2 className="font-bold text-[color:var(--color-text)] text-base mb-3">
            🚀 Empezar
          </h2>
          <div className="space-y-2">
            {[
              {
                label: "Crear o unirme a un grupo",
                href: "/grupos",
                emoji: "👥",
              },
              {
                label: "Cargar mis predicciones",
                href: "/predicciones",
                emoji: "⚽",
              },
              {
                label: "Ver el fixture del Mundial",
                href: "/partidos",
                emoji: "📋",
              },
              ...
              <button
                ...
                className="w-full flex items-center gap-3 bg-surface-2 rounded-2xl px-4 py-3.5 active:scale-95 transition-transform"
              >
                <span className="text-xl">{emoji}</span>
                <span className="text-sm font-semibold text-[color:var(--color-text-2)] flex-1 text-left">
                  {label}
                </span>
                <svg
