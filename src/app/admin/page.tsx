    ...
    <div className="min-h-dvh bg-[#0D1B3E] pb-10">
      ...
      <div className="px-5 pt-14 pb-6">
        <div className="flex items-center justify-between mb-1">
          <div>
            <p className="text-white/40 text-xs tracking-widest">PANEL DE ADMINISTRACIÓN</p>
            <h1 className="text-white font-display text-3xl" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>
              GESTIÓN DE PARTIDOS
            </h1>
          </div>
          <button
        ...
        <div className="mt-4 bg-white/5 rounded-2xl p-4">
          <div className="flex items-center justify-between mb-2">
            <div>
              <p className="text-white font-semibold text-sm">Sincronizar con API-Football</p>
              <p className="text-white/40 text-xs">Actualiza automáticamente todos los resultados disponibles</p>
            </div>
            <button
              onClick={handleSync}
              disabled={syncing}
          ...
          {syncResult && (
            <div className={`rounded-xl px-3 py-2 text-xs mt-2 ${syncResult.error ? 'bg-red-500/20 text-red-300' : 'bg-green-500/20 text-green-300'}`}>
              {syncResult.error
      ...
      {/* Group tabs */}
      <div className="overflow-x-auto flex px-5 gap-2 mb-4" style={{ scrollbarWidth: 'none' }}>
        {GROUPS.map(g => {
          const groupMatchesList = matches.filter(m => m.group_name === g)
          const finishedCount = groupMatchesList.filter(m => m.status === 'finished').length
          return (
            <button
              key={g}
              onClick={() => setActiveTab(g)}
              className={`flex-shrink-0 flex flex-col items-center px-3 py-2 rounded-xl transition-all ${
                activeTab === g ? 'bg-[#003DA5] text-white' : 'bg-white/10 text-white/60'
              }`}
            >
      ...
      <div className="px-5 space-y-3">
        {groupMatches.map((match) => {
          ...
          return (
            <div key={match.id} className="bg-white/5 rounded-2xl overflow-hidden">
              {/* Match header */}
              <div className="flex items-center justify-between px-4 py-2 border-b border-white/5">
                <span className="text-white/40 text-xs">#{match.match_number} · {match.city}</span>
              ...
              <div className="px-4 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 text-center">
                    <span className="text-2xl block mb-1">{match.home_flag}</span>
                    <p className="text-white text-xs font-semibold leading-tight">{match.home_team}</p>
                  </div>
                  ...
                  {/* Away */}
                  <div className="flex-1 text-center">
                    <span className="text-2xl block mb-1">{match.away_flag}</span>
                    <p className="text-white text-xs font-semibold leading-tight">{match.away_team}</p>
                  </div>
                </div>
                  ...
                  {isEditing ? (
                    <>
                      <button
                        ...
                        disabled={saving || editing.home_score === '' || editing.away_score === ''}
